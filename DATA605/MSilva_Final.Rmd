...
```{r}
exponential_pdf <- fitdistr(kaggle$BsmtUnfSF, densfun = "exponential")
```

*The optimal value for $\lambda$ is `r exponential_pdf$estimate`.*

```{r}
samples <- rexp(10000, rate = exponential_pdf$estimate)
```
...
*The original data is not quite exponential, but it's not a bad approximation.  It has a longer tail than the data.*
