...
*There are `r zero_vars` variables that have no correlation with the sale price.  I would be worried be woried about family-wise error because I just ran `r test` hypothesis tests.  The family-wise error rate would be: ${FWER} = 1 - (1 - .2)^`r zero_vars` = 1 - `r .8 ^ zero_vars` = `r 1 - (.8^ zero_vars)`$.  This means there is a really high probability of committing a Type I error.*
...
*This analysis is available on  [https://github.com/mikeasilva/CUNY-SPS](https://github.com/mikeasilva/CUNY-SPS/blob/master/DATA605/MSilva_Final.Rmd).  A walkthough of this analysis is available on [YouTube](https://youtu.be/W5iE-ItRvrQ).*