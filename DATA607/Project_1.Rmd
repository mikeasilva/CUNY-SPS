...
Now to explain my apporach.  I will loop through the character vector.  I will check if the line is made up of dashes.  After reading two dashed lines I will start extracting data.  There are two lines that have useful data.  The first has the player's id, name and match data.  The second line has the player's state and starting score.
...
for (line in lines){
  ...
  if (line == "-----------------------------------------------------------------------------------------"){
    ...
    if (data_line_count == 1){
      ...
      player_name <- str_extract(split_line[2], "[[:alpha:]]+ [[:alpha:]]+")
      player_name <- unlist(strsplit(player_name, " "))
...
```{r add_averages}
player_df <- merge(player_df, averages) %>%
