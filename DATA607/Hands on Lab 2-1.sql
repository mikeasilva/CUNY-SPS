...
-- 2. Using the weather table, concatenate the year, month, and day columns to display a date in the form "3/17/2013".  Consider using the CONCAT() function.
...
SELECT 
    MAX((60 * `hour`) + `minute`) AS `air time`
FROM
    `flights`;
...
SELECT 
    MIN((60 * `hour`) + `minute`) AS `air time`
FROM
    `flights`
        INNER JOIN
    `airlines` ON `airlines`.`carrier` = `flights`.`carrier`
WHERE
    `airlines`.`name` LIKE 'Delta Air Lines%'
...
FROM
    ...
    `airlines` ON `airlines`.`carrier` = `flights`.`carrier`
WHERE
    `airlines`.`name` LIKE 'Alaska Airlines%'
