...
**I'm going to fit the data against a series of tree based models.  I will use caret with vanilla settings to see how well the preform against a tuned PLS model.**
...
```{r}
rf_model <- train(Yield ~ ., data = train_df, method = "rf")
...
```{r}
cubist_model <- train(Yield ~ ., data = train_df, method="cubist")

cubist_predictions <- predict(cubist_model, test_df)
cubist_in_sample <- merge(cubist_model$results, cubist_model$bestTune)
results <- data.frame(t(postResample(pred = cubist_predictions, obs = test_df$Yield))) %>%
  mutate("In Sample RMSE" = cubist_in_sample$RMSE,
         "In Sample Rsquared" = cubist_in_sample$Rsquared,
         "In Sample MAE" = cubist_in_sample$MAE,
         "Model"= "Cubist") %>%
  rbind(results)
```
...
**All tree model's fit the in sample data better than the PLS model.  However they may have overfit the data as the RMSE on the test set is higher for the random forest and boosted tree.  The cubist model is the only on that out preformed the PLS on the test set.  These findings are on vanilla models.  Additional tuning could be used to improve the model's performance**
...
```{r}
varImp(cubist_model, 10)
```

**The manufacturing process variables continue to dominate the list.  Some of the variables on on both lists (in different orders of importance) while there are differences between the two lists:**
