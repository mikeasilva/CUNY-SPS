...
results <- data.frame(t(postResample(pred = pls_predictions, obs = test_df$y))) %>%
  mutate("Model"= "PLS")
```
...
```{r}
knn_model <- train(y ~ ., data = train_df, method = "knn",
                   preProcess = c("center"), 
                   trControl = trainControl("cv", number = 10),
                   tuneLength = 25)
knn_model
...
results <- data.frame(t(postResample(pred = knn_predictions, obs = test_df$y))) %>%
...
```{r}
MARS_grid <- expand.grid(.degree = 1:2, .nprune = 2:15)
MARS_model <- train(y ~ ., data = train_df, method = "earth",
                    tuneGrid = MARS_grid,
                    preProcess = c("center"),
                    tuneLength = 25)
MARS_model
...
results <- data.frame(t(postResample(pred = MARS_predictions, obs = test_df$y))) %>%
...
```{r}
SVM_model <- train(y ~ ., data = train_df, method = "svmRadial",
                   preProcess = c("center"),
                   tuneLength = 25,
                   trControl = trainControl(method = "cv"))
SVM_model
...
results <- data.frame(t(postResample(pred = SVM_predictions, obs = test_df$y))) %>%
...
nnet_model <- train(y ~ ., data = train_df, method = "avNNet",
                    preProcess = c("center"),
                    tuneGrid = nnet_grid,
                    trControl = trainControl(method = "cv"),
                    linout = TRUE,
...
results <- data.frame(t(postResample(pred = nnet_predictions, obs = test_df$y))) %>%
