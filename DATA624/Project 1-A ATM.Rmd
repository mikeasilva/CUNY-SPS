...
df_to_ts <- function(df){
  ...
  ts(df$Cash, start = decimal_date(as.Date(min(df$DATE))), frequency = 365)
}
...
train_test_split <- function(atm_df, atm_number, n_in_test, return_ts=TRUE){
  ...
  train_eval_test_ts <- list(train = train, test = test, evaluation = eval_ts, short_train = short_train)
