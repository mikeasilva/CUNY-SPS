...
df_to_ts <- function(df){
  ### Converts ATM data frames into ts objects
  # Args:
  #     df (data.frame): The ATM data frame 
  # Returns:
  #     ts (Time-Series): An ATM time series object
  ts(df$Cash, start = decimal_date(as.Date(min(df$DATE))), frequency = 7)
  #msts(df$Cash, seasonal.periods=c(7, 365.25))
}

train_test_split <- function(atm_df, atm_number, n_in_test, return_ts=TRUE){
  ...
  # Returns:
  #     train_eval_test_ts (list): Data split into sets (train, eval and test).
  ...
  # Split the data frame into training, eval and test sets
  full <- df
  test <- tail(df, n_in_test)
  train <- head(df, -n_in_test)
  eval_ts <- tail(train, n_in_test)
  short_train <- head(df, -(2 * n_in_test))
  
  # Tranform into ts objects
  if(return_ts){
    full <- df_to_ts(full)
    train <- df_to_ts(train)
    test <- df_to_ts(test)
    eval_ts <- df_to_ts(eval_ts)
    short_train <- df_to_ts(short_train)
  }
  
  # Return the data to the user
  train_eval_test_ts <- list(train = train, test = test, evaluation = eval_ts, short_train = short_train, full = full)
  
  return(train_eval_test_ts)
}

# Get time series objects
atm_1_ts <- train_test_split(adjusted_atm, 1, forecast_window)
atm_2_ts <- train_test_split(adjusted_atm, 2, forecast_window)
atm_3_ts <- train_test_split(adjusted_atm, 3, forecast_window)
atm_4_ts <- train_test_split(adjusted_atm, 4, forecast_window)
# Get data frame objects
atm_1_df <- train_test_split(adjusted_atm, 1, forecast_window, return_ts = FALSE)
atm_2_df <- train_test_split(adjusted_atm, 2, forecast_window, return_ts = FALSE)
atm_3_df <- train_test_split(adjusted_atm, 3, forecast_window, return_ts = FALSE)
atm_4_df <- train_test_split(adjusted_atm, 4, forecast_window, return_ts = FALSE)
```
...
atm_1_df$train %>%
  ...
  ggplot(aes(x=DATE, y=Cash, color=`Day of Week`)) +
  geom_point() +
  #geom_line() +
...
Saturday looks fairly consistent.  Other days (Tuesday and Thursday) show recent changes in their trend.

```{r}
autoplot(hw(atm_1_ts$train, h=forecast_window))
```

```{r}
atm_1_holt <- holt(atm_1_ts$train, h=forecast_window)
autoplot(atm_1_holt) + autolayer(atm_1_ts$test)
```
...
```{r}
atm_1_mean <- mean(atm_1_df$train)
atm_1_last_observation <- tail(atm_1_df$train, 1)$Cash
atm_1_mean <- mean(atm_1_df$train$Cash)
atm_1_results <- 
  atm_1_prophet$forecast %>%
  mutate(DATE = as.Date(ds), prophet_yhat = yhat) %>%
  select(DATE, prophet_yhat) %>%
  mutate(naive_yhat = atm_1_last_observation, 
         mean_yhat = atm_1_mean) %>%
  merge(atm_1_df$evaluation) %>%
  rename(actual = Cash)
atm_1_results$holt <- tail(atm_1_holt$fitted, forecast_window)
```
```{r}
day_of_week_averages <- atm_1_df$train %>%
  mutate(day_of_week = wday(DATE)) %>%
  group_by(day_of_week) %>%
  summarise(yhat = median(Cash))
temp <- atm_1_df$test %>%
  mutate(day_of_week = wday(DATE)) %>%
  merge(day_of_week_averages) %>%
  arrange(DATE) %>%
  select(DATE, yhat)
atm_1_results$day_of_week_model <- temp$yhat
```
...
rf_fit <- randomForest(Cash~., rf_data(atm_1_df$train))
predict(rf_fit, rf_data(atm_1_df$test))

atm_1_results$rf <- predict(rf_fit, rf_data(atm_1_df$test))
```

```{r}
lm_fit <- lm(Cash~day_of_week, rf_data(atm_1_df$train))
atm_1_results$lm <- predict(lm_fit, rf_data(atm_1_df$test))
```
