...
$$\sigma^2=\frac{\Sigma (x-E(x))^2}{N} = \frac{(5-7.2)^2+(6-7.2)^2+(7-7.2)^2+(8-7.2)^2+(9-7.2)^2}{5}$$
$$\sigma^2= \frac{4.84+1.44+0.04+0.64+3.24}{5}=\frac{10.2}{5}=2.04$$

```{r}
v <- sum((x-ex)**2)/length(x)
v
```
#### Step 3

Find the standard deviation. Round your answer to one decimal place.
$$\sigma = \sqrt{\sigma ^2} = \sqrt{2.04} \approx 1.4$$
```{r}
v**(1/2)
```
