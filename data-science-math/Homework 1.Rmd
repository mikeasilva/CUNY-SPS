...
Applying the quotient rule where $f(x)=5\sqrt{x}$ and $g(x)=x^3+1$

$$f'(x) = \frac{\Big(\frac{5}{2\sqrt{x}}\Big)(x^3+1)-(3x^2(5x^\frac{1}{2}+7))}{(x^3+1)^2}$$
Simplified to 
$$f'(x) = -\frac{42x^\frac{5}{2} + 25x^3 - 5}{2\sqrt{x}\cdot (x^6+2x^3 + 1)}$$
Again checking with R:
```{r}
f11 <- function(x){(7+5*x**(1/2))/(1+x**3)}
Deriv(f11)
# Evaluate at 5
fprime11 <- function(x){(-42*x**(5/2)+25*x**3-5)/((2*x**(1/2))*(x**6+2*x**3+1))}
fprime11(5)
fprime11 <- Deriv(f11)
fprime11(5)
```
...
$$f'(t)=$$
```{r}
f13 <- function(t){(550*t**2)/(t+15)**(1/2)}
Deriv(f13)
```
