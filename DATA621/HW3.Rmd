...
We have been given a dataset with `r nrow(df)` records summarizing a attributes of various neighborhoods of a major city.  Each record in the training dataset has a response variable that is 1 when the neighborhood's crime rate is above the median and 0 when it is not.  In all there are `r ncol(df)-1` predictors.  We will construct a logistic regression model to predict the classification of the evaluation data set.
...
```{r}
evaluation$target <- ifelse(predict.glm(model1, evaluation, "response") < 0.5, 0, 1)
evaluation %>%
  write.csv(., "./data/hw3-predictions.csv", row.names = F)
```