...
1.  What type of plot would you use to display the relationship between `runs` 
    and one of the other numerical variables? **A scatter plot.** Plot this 
    relationship using the variable `at_bats` as the predictor. 
    ```{r}
    plot(mlb11$at_bats, mlb11$runs)
    ```
    
    Does the relationship look linear? **Yes it looks somewhat linear.** If you knew
    a team's `at_bats`, would you be comfortable using a linear model to predict the 
    number of runs? **I don't know if I would.** 
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    strength of the relationship as well as any unusual observations. **The relationship is positive (upward sloping) and mostly linear and not very strong.  There are a few of outliers.**
...
4.  Fit a new model that uses `homeruns` to predict `runs`. Using the estimates 
    ...
    and its home runs? **$\hat{y} = 415.2389 + 1.8345 * homeruns$**
    ```{r}
...
5.  If a team manager saw the least squares regression line and not the actual 
    ...
    $\hat{y} = -2789.2429 + 0.6305 * atbats = -2789.2429 + 0.6305 * 5578 = -2789.2429 + 3516.929 = 727.6861$
    Is this an overestimate or an underestimate, and by how much? In other words, 
    what is the residual for this prediction? 
    ```{r message=FALSE, echo=FALSE}
    library(dplyr)
    test <- mlb11 %>%
      mutate(diff = abs(at_bats - 5578)) %>%
      mutate(closest = min(diff)) %>%
      filter(closest == diff) %>%
      select(at_bats, runs) 
    ```
    **The closest data we have is `r test$at_bats`.   For this case the model predicts `r round(predict(m1, test), 0)` runs and the actual was `r test$runs`.  The residual is `r round(test$runs - predict(m1, test), 0)` runs which indicates the model was an overestimate.  It would be similar to 5,578 at bats.**
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    about the linearity of the relationship between runs and at-bats? **No there is no apparent pattern in the residuals.**
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    normal residuals condition appear to be met?  **Yes it appears to be normal**
...
-   Choose another traditional variable from `mlb11` that you think might be a 
    ...
    **I think homeruns would be a good predictor because it gets to the skill of the player.  new_obs would probably be better but it is not a traditional variable. **
    
    ```{r}
    plot(mlb11$homeruns, mlb11$runs)
    ```
    
    **Yes this appears to be a positive linear relationship.**
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    summary(m3)
    ```
    **The $R^2$ for this model is `r summary(m3)$r.squared` which is higher than `r summary(m1)$r.squared`.  It does a better job of predicting.  You can tell because the $R^2$ is higher and the residual standard error is lower.**
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    ```{r}
    highest_r_squared <- 0
    # Loop through all the traditional variables
    for(v in c('at_bats', 'hits', 'homeruns', 'bat_avg', 'strikeouts', 'stolen_bases', 'wins')){
      temp <- mlb11[,c('runs', v)]
      model <- lm(runs ~ ., data = temp)
...
-   Now examine the three newer variables. These are the statistics used by the 
    ...
    ```{r}
    # Loop through all the Moneyball variables
    best_r_squared <- summary(best_model)$r.squared
    for(v in c('new_onbase', 'new_slug', 'new_obs')){
      temp <- mlb11[,c('runs', v)]
      model <- lm(runs ~ ., data = temp)
      r_squared <- summary(model)$r.squared
      if(r_squared > best_r_squared){
        message(paste(v, "outpreforms", best_var, "(", round(r_squared, 2), " > ", round(best_r_squared, 2), ")"))
        best_model <- model
        best_data <- temp
      }
    }
...
-   Check the model diagnostics for the regression model with the variable you 
    ...
    **Linearity : No pattern to the residuals.** 
    
    **Constant variability : Yes Sir!** 
    
    ```{r}
    plot(best_model$residuals ~ mlb11$new_obs)
    abline(h = 0, lty = 3)
    ```
