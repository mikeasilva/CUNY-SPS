...
Finding affordable housing is a real challenge throughout the country.  The Rochester, New York metro is acclaimed as a place with affordable housing. Recent data shows 81% of home in the Rochester Metro are affordable [@nahb_hoi].  The median sales price for a home in Rochester is $146,000.  This is about half as much as the national average ($268,000).  With the high family income, housing proves to be affordable.
...
After filtering there are `r prettyNum(nrow(df), big.mark = ",", scientific = FALSE)` observations.  This is the final count of cases for the study.  The next step is to create the categorical variable from the construction year.
...
The study data under represents the two highest and two lowest categories. It also over represents the $100,000 to $149,999 price range. This could indicate different segments of the housing stock “moved” faster/more than other segments.

This data above raise some concern.  The data appears skewed.  The following density plot illustrates the skew.  The x-axis is stopped at half a million even though the largest sale price in the data set is `r round(max(df$Price) / 1000000, 1)` million dollars.
...
There are extreme housing prices in the data set.  The data is also not normal.  To correct this, the price will be on a log scale and outliers will be removed.  
...
It is clear to see that price and size are related.  The relationship between price and era is not as clear  The following visualization and summary table examines this relationship:
...
The two explanatory variables might be related.  It is reasonable to believe that home increased in size over time.  The plot below will display how the size and era are related.
...
This model's coefficients are a little hard to interpret given that they are on a log scale.  Yet one observes that all the coefficients are statistically significant.  The square footage of living space (size) is positively correlated with the home price.  Also the later the home is built is positively correlated with the price.  60% of the variability in the log of the sales price is explained by these two variables.
...
There are `r nrow(testing[testing$Accuracy=="Over Estimate", ])` cases where the model over estimates and `r nrow(testing[testing$Accuracy=="Under Estimate", ])` where it under estimates the home price. `r nrow(testing[testing$Accuracy=="Within CI", ])` are in within the confidence interval.  Let's examine to what degree it over/under estimates.
...
There is a wide variety in the errors.  Let's look at the distribution of the error in absolute terms as a share of the total price.  It would be nice to see errors in the 5% range
...
Only `r round((nrow(testing[testing$Error_Share < 5, ]) / nrow(testing)) * 100, 0)`% of the testing set observations have an error which is +/-5% of the estimate.

## 5. Conclusion

This study set out to see if the size of a home and the era in which it was built could be used to get an estimate of the sale price. While these variables seem to be relevant, it appears that it is more complicated. Future research could expand the model.  A location variable would be a next step.  A different modeling approach may be employed as the relationship appears to be non-linear.
## 6. References