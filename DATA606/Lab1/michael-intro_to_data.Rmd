...
source('~/Documents/CUNY-SPS/DATA606/Lab1/more/cdc.R')
```
...
**There are 20K records with 9 variables.  height weight wtdesire and age are numeric and continuous. gender and genhlth are categorical.  exerany, hlthplan and smoke100 are categorical (0=no and 1=yes).**
...
**The box plot shows the distribution of the data.  It shows the IQR, median, and outliers.  The above box plot shows that the individuals with poorer general health are more likely to have a higher BMI than those with better health.**

```{r}
boxplot(bmi ~ cdc$gender)
```

**I picked gender because general health status was already taken.  I would expect a difference based on the gender because men and women are different.  The plot shows a greater variation among the women than the men.**
...
abline(lm(cdc$weight ~ cdc$wtdesire), col="#357FAA", lty='longdash', lwd=2)
```
...
**It is a continous variable.  If wdiff is equal to 0 it means that their desired weight is their current weight.  If wdiff is positive they desire to put on weight.  If it is negative they desire to weight less.**
...
**The data is left skewed and clustered around -10 to -15.  There is a few extreme values but in general people want to loose a bit of weight.**
...
summary(cdc[cdc$gender == "m",]$wtdesire)
summary(cdc[cdc$gender == "f",]$wtdesire)

boxplot(cdc$wtdesire ~ cdc$gender)
```

**In order to see how they different genders viewed their weight I decided to look at the weight they desired.  It would stand as a proxy for the "ideal" man or woman.  As one would expect the "ideal" male's weight is heavier than the "ideal" female's.**
...
nrow(cdc[cdc$weight>=lower & cdc$weight<=upper,]) / nrow(cdc)
```

**The mean is roughly 170 with a standard deviation of about 40.  About 71% are within one standard deviation of the mean.**
