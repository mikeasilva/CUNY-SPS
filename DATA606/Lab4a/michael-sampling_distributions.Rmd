...
1.  Describe this population distribution. **It is long-tailed or right skewed distribution.**
...
2.  Describe the distribution of this sample. How does it compare to the 
    distribution of the population? **It is similar to the population distribution.**
    ```{r, echo=FALSE, message=FALSE}
    library(ggplot2)
    library(dplyr)
    df <- data.frame(area=area, Group=c("Population"))
    df <- data.frame(area=samp1, Group=c("Sample")) %>%
      rbind(df) %>%
      mutate(Group = as.factor(Group))
    ggplot(df) + geom_density(aes(x=area, fill=Group, alpha=0.2)) + guides(alpha="none")
    ```
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    mean of `samp2` compare with the mean of `samp1`?  **They are pretty close to each other.** Suppose we took two 
    more samples, one of size 100 and one of size 1000. Which would you think 
    would provide a more accurate estimate of the population mean? **The sample with more observations.**
    ```{r samp2}
    samp2 <- sample(area, 50)
    message(paste("Population:", mean(area)))
    message(paste("Sample of 50 #1:", mean(samp1)))
    message(paste("Sample of 50 #2:", mean(samp2)))
    message(paste("Sample of 100:", mean(sample(area, 100))))
    message(paste("Sample of 1000:", mean(sample(area, 1000))))
    ```
...
4.  How many elements are there in `sample_means50`?  **`r length(sample_means50)`** Describe the sampling 
    ...
    means?  **Its center is about `r round(mean(sample_means50), 0)`.  The distribution would still be normally distributed but it should haved a smaller spread.**
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    sample_means_small
    ```
    How many elements are there in this object called `sample_means_small`? **`r length(sample_means_small)`**
    What does each element represent? **The mean area of 50 randomly selected homes in Ames.**
...
6.  When the sample size is larger, what happens to the center?  **The center becomes closer to the population mean.**  What about the spread? **The spread narrows.**
...
-   Take a random sample of size 50 from `price`. Using this sample, what is 
    ...
    ```{r}
    avg_price <- mean(sample(price, 50))
    avg_price
    ```
...
-   Since you have access to the population, simulate the sampling 
    ...
    in a vector called `sample_means50`. 
    ```{r}
    for(i in 1:5000){
    ...
    **The distribution is normally distributed.**
    ...
    ```{r, echo=FALSE}
    ggplot() + geom_histogram(aes(sample_means50), bins=100) + geom_vline(xintercept=mean(sample_means50), linetype="dashed", color="red")
    ```
    
    Based on this sampling distribution, what would you guess the mean home price of the population to be? **Roughly $180K.**
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    ```{r}
    sample_means150 <- rep(NA, 150)
    for(i in 1:5000){
      sample_means150[i] <- mean(sample(price, 150))
    }
    ```
    Describe the shape of this sampling distribution, and compare it to the 
    sampling distribution for a sample size of 50.  Based on this sampling 
    distribution, what would you guess to be the mean sale price of homes in Ames?
    ```{r}
    df <- data.frame(price=sample_means150, N=c("150"))
    df <- data.frame(price=sample_means50, N=c("50")) %>%
      rbind(df) %>%
      mutate(N = as.factor(N))
    ggplot(df) + geom_density(aes(x=price, fill=N, alpha=0.2)) + guides(alpha="none")
    ``` 
    
    **Roughly $180K still but I'm more certain of it.**
  
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  
    **The one with 150 observations in the sample.**
    ...
    **As large as we can reasonably get.**
