...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    **A streak length of 1 means one hit followed by one miss.  A streak of length zero means one miss.**
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    **The typical (mode) streak has a length of zero.  Kobe's longes streak of baskets is four.**
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    table(sim_streak)
    ```
    **The typical streak length is zero.  The longest streak length is four.**
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    **I would expect it to be similar but not exactly the same.**
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    **They are similar distributions.  This suggests that Kobe Bryant is very similar to the simulated independent shooter.  The hot hand model does not explain Kobe's shooting percentage.  Additional simulations should be run before making a firm conclusion about the hot hand model.**
    
    ```{r, warning=FALSE, message=FALSE}
    library(dplyr)
    library(ggplot2)
    viz_df <- data.frame(streak = kobe_streak) %>%
      mutate(case = "Kobe")
    viz_df <- data.frame(streak = sim_streak) %>%
      mutate(case = "Simulation") %>%
      rbind(viz_df)
    
    ggplot(viz_df, aes(x=streak, color=as.factor(case))) + geom_density() + theme(axis.title = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank(), legend.title = element_blank(), legend.position = "bottom", plot.title = element_text(hjust = 0.5)) + ggtitle("Streak Distributions by Case")
    ```
