...
1.  What are the cases in this data set? **Live births** How many cases are there in our sample? **`r nrow(nc)`**
...
`length`. **Sample silzes are significantly large.**
```{r}
by(nc$weight, nc$habit, length)
```

4.  Write the hypotheses for testing if the average weights of babies born to 
smoking and non-smoking mothers are different. **They are $H_0 = \mu_{nonsmoker} - \mu_{smoker} = 0$ and $H_A = \mu_{nonsmoker} - \mu_{smoker} != 0$**
...
mothers. Use a method of your choice, and explain how your method works. **My method looks at the oldest young mother and the youngest mature mother.  It looks like 35 is the cutoff.**
```{r}
young_mothers <- nc[nc$mature == "younger mom",]
mature_mothers <- nc[nc$mature != "younger mom",]
max(young_mothers$mage)
min(mature_mothers$mage)
```
...
plain language. **Does the amount of weight gained depend on the gender of the baby.  The null hypothesis is that the baby's gender does not have any impact on the weight gained ($H_0 = \mu_{boy} - \mu_{girl} = 0$).  The alternative is that the gender of the baby has an effect on the amount of weight gained ($H_0 = \mu_{boy} - \mu_{girl} != 0$).  There is no evidence to suggest that the gender of the child has any bearing on the amount of weight gained by the mother.**
