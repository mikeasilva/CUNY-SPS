...
bdims %>%
      ...
      select(hgt, sex) %>%
      ggplot(., aes(x = hgt, color=sex, fill=sex)) + geom_histogram() + facet_grid(sex ~ .) + guides(fill=FALSE, color=FALSE)
    ```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
    **Yes it looks nearly normally distributed to me.**
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    **Most of the points fall upon the line.  It looks very similar to the preceeding plot**
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    **Yes they look similar to the plots created for the simulated data.  They seem nearly normal.**

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    ```{r qqnormsim2, eval=TRUE}
qqnormsim(fdims$wgt)
```
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    **1) Given the mean height of a male is `r round(mean(mdims$hgt), 0)` cm, what is the probability that a female is taller than the average male?**
    
    **The theoretical probability is: `r 1 - pnorm(q = mean(mdims$hgt), mean = fhgtmean, sd = fhgtsd)`**
    
    **The emperical probability is: `r sum(fdims$hgt > mean(mdims$hgt)) / length(fdims$hgt)`**
    
    **2) Given that the mean weight of a male is `r round(mean(mdims$wgt), 0)` kg, what is the probability that a female is heavier than the average male?**
    
    **The theoretical probability is: `r 1 - pnorm(q = mean(mdims$wgt), mean = fwgtmean, sd = fwgtsd)`**
    
    **The emperical probability is: `r sum(fdims$wgt > mean(mdims$wgt)) / length(fdims$wgt)`**
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?  **Because they are variable with discrete units not continuous.  Thats what gives them the stepwise pattern.**
