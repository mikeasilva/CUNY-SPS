...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    sample) or *population parameters*? **They read like they are population parameters, however Table 6 states it's the percent of respondents.  It appears that it is sample statistics.**
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?  **The population represents a random sample, independent, and sufficiently large to represent the population of their country. Since they were surveyed via face-to-face interviews, telephone or online it seems they did not exclude a population.**
...
3.  What does each row of Table 6 correspond to? What does each row of 
    `atheism` correspond to?**A response to the survey.**
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    responses. Does it agree with the percentage in Table 6? **Yes** If not, why?
...
us12 %>%
  group_by(response) %>%
  summarise(count = n(), total = nrow(us12)) %>%
  mutate(share = round(count / total, 2)) %>%
  kable() %>%
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    Are you confident all conditions are met? **The sample size is arround 1,000 which is the right size (less than 10% of the population).**
...
6.  Based on the R output, what is the margin of error for the estimate of the 
    proportion of the proportion of atheists in US in 2012? **`r 0.0499 - 0.0364`**
...
7.  Using the `inference` function, calculate confidence intervals for the 
    ...
    ```{r}
    china_12 <- subset(atheism, nationality == "China" & year == "2012") 
    inference(china_12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
    ```
    
    ```{r}
    fiji_12 <- subset(atheism, nationality == "Fiji" & year == "2012") 
    inference(fiji_12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
    ```
...
8.  Describe the relationship between `p` and `me`. **It is a semi-cirle ranging from 0 to 1 with the maximum at 0.5.**
...
9.  Describe the sampling distribution of sample proportions at $n = 1040$ and 
    ...
    *Hint:* Remember that R has functions such as `mean` to calculate summary
    statistics. **Symetrical, centered at `r mean(p_hats)` and ranging from `r min(p_hats)` to `r max(p_hats)`.**
...
10. Repeat the above simulation three more times but with modified sample
    ...
    |   n  |  p   |      Center       |                Range                | SD |
    |------|------|-------------------|-------------------------------------|----|
    | 1040 | 0.1  | `r mean(p_hats)`  | `r min(p_hats)` to `r max(p_hats)`  |`r sd(p_hats)`|
    | 400  | 0.1  | `r mean(p_hats2)` | `r min(p_hats2)` to `r max(p_hats2)`|`r sd(p_hats2)`|
    | 1040 | 0.02 | `r mean(p_hats3)` | `r min(p_hats3)` to `r max(p_hats3)`|`r sd(p_hats3)`|
    | 400  | 0.02 | `r mean(p_hats4)` | `r min(p_hats4)` to `r max(p_hats4)`|`r sd(p_hats4)`|
    
    **The larger the n the closer to the p the center is and the smaller the standard deviation is.  The p shifts the histogram along the x axis.**  
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    proceed with inference and report margin of errors, as the reports does?**No because the sample margin of error is not the same thing as the population margin of error.**
...
-   Answer the following two questions using the `inference` function. As 
    ...
    *Hint:* Create a new data set for respondents from Spain. Form
    ...
    years, and determine whether they overlap. **H0: $\text{Index}_2005$ = $\text{Index}_2012$ and Ha: $\text{Index}_2005$ != $\text{Index}_2012$**
    ```{r}
    ...
    ```
    **The confidence intervals overlap so we cannot reject the null hypothesis, therefore there is not convinsing evidence that Spain's atheism index has changed.**

    **b.** Is there convincing evidence that the United States has seen a
    change in its atheism index between 2005 and 2012? **H0: $\text{Index}_2005$ = $\text{Index}_2012$ and Ha: $\text{Index}_2005$ != $\text{Index}_2012$**
    ```{r}
    ...
    ```
    **There is no overlap in the confidence intervals which suggests that there is evidence that the United States has experienced a change in the proportion of the population claiming to be atheist between 2005 and 2012.**
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error. **There is 39 countries on the list so we would expect 2 (=39 $\times$ 0.05) countries have a change detected simply by chance.**
...
-   Suppose you're hired by the local government to estimate the proportion of 
    ...
    *Hint:* Refer to your plot of the relationship between $p$ and margin of 
    error. Do not use the data set to answer this question.**Since we have no idea about $p$ we would assume 0.5 for $p$.**
    $\text{point estimate} \pm \text{critical value} \times \sqrt{\frac{p(1-p)}{n}}$
    **Substituting in what we know and solve for n.**
    
    $0.01 = 1.96 \times \sqrt{\frac{0.25}{n}} \rightarrow 0.01^2 = 1.96^2 \times \frac{0.25}{n} \rightarrow 0.0001 = \frac{3.8416 \times 0.25}{n} \rightarrow n = \frac{0.9604}{.0001} \rightarrow n = 9604$ 
