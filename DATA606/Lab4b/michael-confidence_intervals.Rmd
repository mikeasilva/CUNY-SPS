...
"typical" size within your sample? **The "typical" size is `r mean(samp)`.** Also state precisely what you interpreted "typical" to mean.  **In this case typical means the mean of the mean of the sample.**

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not? **I would expect it to be similar but not identical to mine.  Similar because they were drawn from the same population.  But not identical because they are selected using a random process.**
...
met for this to be true? **Obervations are IID and sample is random.**

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2. **If 100 confidence intervals were taken, the population mean would fall within the CI about 95% of the time.**
...
```{r, null_prefix=TRUE, echo=FALSE, results='asis'} 
if(contains(lower, upper, pop_mean)) {
  paste('**Yes it does.', round(pop_mean,0), 'is between', round(lower,0), 'and', round(upper,0), '.**')
} else{
  paste('**No it does not.', round(pop_mean,0), 'is not between', round(lower,0), 'and', round(upper,0), '.**')
}
```
...
capture the true population mean? **95%** Why? **Due to the central limit theorem.**If you are working in this lab in a 
...
-   Using the following function (which was downloaded with the data set), 
    ...
    n_within <- 0
    for(i in 1:50){
      lower <- lower_vector[i]
      upper <- upper_vector[i]
      if(contains(lower, upper, pop_mean)){
        n_within <- n_within + 1
      }
    }
    share_95 <- n_within / 50
    share_95
    ```
    ...
    ```{r, null_prefix=TRUE, echo=FALSE, results='asis'} 
      if(share_95 >= .95){
        ...
        print('**No.  This is one of the 5% of the times it does not.**')
      }
    ```
    
-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value? **For a 90% CI the critical value is 1.64.**
...
-   Calculate 50 confidence intervals at the confidence level you chose in the 
    ...
    plot_ci(lower_vector, upper_vector, pop_mean)
    ...
    **There are more CI's that are red (contain the population mean) than above.**
